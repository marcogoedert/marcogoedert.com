...
describe("extractQueryFromGoodreadsSlug", () => {
  it("extracts title from dot format (strip numeric prefix)", () => {
    expect(extractQueryFromGoodreadsSlug("1536545.The_Likeness")).toBe(
      "The Likeness",
    );
  });
  ...
  it("extracts title from hyphen format", () => {
    expect(
      extractQueryFromGoodreadsSlug(
        "58784475-tomorrow-and-tomorrow-and-tomorrow",
      ),
    ).toBe("tomorrow and tomorrow and tomorrow");
  });

  it("replaces dots, underscores, hyphens with spaces after prefix strip", () => {
    expect(extractQueryFromGoodreadsSlug("123.Some_Book-Title")).toBe(
      "Some Book Title",
    );
  });
