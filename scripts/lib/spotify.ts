...
export async function fetchSpotifyAlbum(
  ...
  if (!res.ok)
    throw new Error(`Spotify album fetch failed: ${res.status} ${res.url}`);
...
export async function searchAndSelectSpotify(
  ...
  if (!res.ok)
    throw new Error(`Spotify search failed: ${res.status} ${res.url}`);
  ...
  return select({
    message: "Select:",
    choices: allItems.map((r) => ({
      name: r.label,
      value: r as SpotifyAlbumData,
    })),
  });
}
...
export async function fetchSpotifyTrack(
  ...
  if (!res.ok)
    throw new Error(`Spotify track fetch failed: ${res.status} ${res.url}`);
