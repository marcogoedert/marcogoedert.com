...
async function fetchMovieDetails(movie: MovieResult, originalUrl: string, apiKey: string): Promise<TmdbTitleData> {
  const creditsRes = await fetch(
    `https://api.themoviedb.org/3/movie/${movie.id}/credits`,
    { headers: { Authorization: `Bearer ${apiKey}` } }
  )
  if (!creditsRes.ok) throw new Error(`TMDB credits failed: ${creditsRes.status} ${creditsRes.url}`)
  return parseTmdbMovie(movie, (await creditsRes.json()) as CreditsResponse, originalUrl)
}

async function fetchTvDetails(tv: TvResult, originalUrl: string, apiKey: string): Promise<TmdbTitleData> {
  ...
  return parseTmdbTv(tv, (await detailsRes.json()) as TvDetailsResponse, originalUrl)
}
...
export async function fetchTmdbTitle(
  ...
  if (findData.movie_results.length > 0) return fetchMovieDetails(findData.movie_results[0], originalUrl, apiKey)
  if (findData.tv_results.length > 0) return fetchTvDetails(findData.tv_results[0], originalUrl, apiKey)
  throw new Error(`No results found for ${imdbId}`)
}

export async function searchAndSelectTmdb(query: string, apiKey: string): Promise<TmdbTitleData> {
  ...
  const chosen = items.length === 1
    ...
    : await select({
        message: "Select:",
        choices: items.map((r) => ({ name: formatTmdbSearchLabel(r), value: r })),
      })

  const originalUrl = chosen.media_type === "movie"
    ? `https://www.themoviedb.org/movie/${chosen.id}`
    : `https://www.themoviedb.org/tv/${chosen.id}`

  if (chosen.media_type === "movie") {
    return fetchMovieDetails({ id: chosen.id, title: chosen.title ?? "Unknown", poster_path: chosen.poster_path }, originalUrl, apiKey)
  }
  return fetchTvDetails({ id: chosen.id, name: chosen.name ?? "Unknown", poster_path: chosen.poster_path }, originalUrl, apiKey)
}
