...
async function addCommand(urlOrQuery?: string): Promise<void> {
  ...
  if (urlOrQuery && isUrl(urlOrQuery)) {
    ...
    if (detected.type === "listens") {
      ...
      if (detected.spotifyType === "track") {
        fetched = await fetchSpotifyTrack(
          detected.id,
          process.env.SPOTIFY_CLIENT_ID!,
          process.env.SPOTIFY_CLIENT_SECRET!,
        );
      } else {
        fetched = await fetchSpotifyAlbum(
          detected.id,
          process.env.SPOTIFY_CLIENT_ID!,
          process.env.SPOTIFY_CLIENT_SECRET!,
        );
      ...
      const bookQuery = await input({
        message: "Search query (add author for better results)",
    ...
    const queryMessage =
      contentType === "reads"
        ? "Search query (add author for better results)"
        : "Search query";
    ...
    if (contentType === "listens") {
      checkEnvVars(["SPOTIFY_CLIENT_ID", "SPOTIFY_CLIENT_SECRET"]);
      fetched = await searchAndSelectSpotify(
        query,
        process.env.SPOTIFY_CLIENT_ID!,
        process.env.SPOTIFY_CLIENT_SECRET!,
      );
  ...
  if (contentType === "reads") {
    const rd = await input({
      message: 'Read date (e.g. "January 2026", leave blank to skip)',
    });
    if (rd) readDate = rd;
  }

  const ratingInput = await input({
    message: "Rating 1-5 (leave blank to skip)",
  });
  ...
  const entry: IMediaItem = {
    ...
    ...(contentType === "reads" &&
      fetched.publishedYear !== undefined && {
        publishedYear: fetched.publishedYear,
      }),
    ...(contentType === "reads" && readDate !== undefined && { readDate }),
    ...(contentType === "listens" &&
      fetched.albumName !== undefined && { albumName: fetched.albumName }),
    ...(contentType === "listens" &&
      fetched.duration !== undefined && { duration: fetched.duration }),
  };
...
async function editCommand(query: string): Promise<void> {
  ...
  const coverVal = await input({
    message: "Cover image URL (freeform string)",
    default: entry.coverImage,
  });

  let publishedYear = entry.publishedYear;
  let readDate = entry.readDate;
  if (file === "reads") {
    const pyInput = await input({
      message: "Published year",
      default: entry.publishedYear?.toString() ?? "",
    });
    publishedYear = pyInput ? parseInt(pyInput, 10) : undefined;
    const rdInput = await input({
      message: "Read date",
      default: entry.readDate ?? "",
    });
    readDate = rdInput || undefined;
  }

  const ratingInput = await input({
    message: "Rating 1-5 (blank = keep existing)",
    default: "",
  });
...
if (
  process.argv[1]?.endsWith("media.ts") ||
  process.argv[1]?.endsWith("media.js")
) {
